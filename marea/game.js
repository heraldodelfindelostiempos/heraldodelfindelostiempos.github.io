...
function paintFinish(p){
 ctx.save();
 for(let i=0;i<110;i++){const x=((seed(i+23)*W-s.world*.028)%W+W)%W,y=seed(i+97)*385,length=9+seed(i+191)*86;ctx.globalAlpha=.035+seed(i+310)*.085;line([[x,y],[x+length,y-2-seed(i+11)*7]],i%3?p.foam:p.high,1+seed(i+400)*3)}
 for(let i=0;i<170;i++){const x=((seed(i+553)*W-s.world*(.15+seed(i+10)*.2))%W+W)%W,y=top(x)+28+seed(i+661)*Math.min(depth(x),360),length=10+seed(i+93)*66;ctx.globalAlpha=.045+seed(i+818)*.11;line([[x,y],[x+length,y+seed(i+45)*9-4]],i%4?p.foam:p.deep,1+seed(i+5)*4)}
 ctx.globalAlpha=1;const shade=ctx.createRadialGradient(W*.51,H*.43,210,W*.51,H*.43,850);shade.addColorStop(0,'rgba(4,9,23,0)');shade.addColorStop(1,'rgba(4,9,23,.32)');ctx.fillStyle=shade;ctx.fillRect(0,0,W,H);ctx.restore()
}
function render(){
