...
let audioGraph=null;
function setupAudio(){if(audioGraph)return;const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)return;try{const ac=new Audio(),master=ac.createGain(),filter=ac.createBiquadFilter(),source=ac.createMediaElementSource(music);filter.type='lowpass';source.connect(filter).connect(master);const pads=[110,165].map((freq,i)=>{const osc=ac.createOscillator(),gain=ac.createGain();osc.type=i?'sine':'triangle';osc.frequency.value=freq;gain.gain.value=.015;osc.connect(gain).connect(master);osc.start();return {osc,gain}});const buffer=ac.createBuffer(1,ac.sampleRate*2,ac.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<data.length;i++)data[i]=Math.random()*2-1;const noise=ac.createBufferSource(),noiseFilter=ac.createBiquadFilter(),rainGain=ac.createGain();noise.buffer=buffer;noise.loop=true;noiseFilter.type='bandpass';noiseFilter.frequency.value=630;noiseFilter.Q.value=.36;rainGain.gain.value=0;noise.connect(noiseFilter).connect(rainGain).connect(master);noise.start();master.gain.value=0;master.connect(ac.destination);music.volume=1;audioGraph={ac,master,filter,pads,rainGain};ac.resume().catch(()=>{})}catch(e){audioGraph=null}}
function updateAudio(p,dt){if(!audioGraph){if(!music.paused&&!music.muted)music.volume=Math.min(.32,music.volume+dt*.055);return}const {ac,master,filter,pads,rainGain}=audioGraph,t=ac.currentTime,phase=p.phase,storm=p.rain,night=p.night,muted=music.muted||music.paused;master.gain.setTargetAtTime(muted?0:.42,t,.24);filter.frequency.setTargetAtTime(5200-night*2200-storm*2250,t,.8);const dusk=smooth((phase-.75)/.3)*(1-smooth((phase-1.8)/.3)),notes=[110-12*dusk-28*night-37*storm,164.81-18*dusk-41*night-54*storm];pads.forEach(({osc,gain},i)=>{osc.frequency.setTargetAtTime(notes[i],t,1.4);gain.gain.setTargetAtTime((i?.012:.018)*(1+night*.55+storm*.25),t,.8)});rainGain.gain.setTargetAtTime(storm*.045,t,.5)}
let s;
...
function background(p){const gradient=ctx.createLinearGradient(0,0,0,450);gradient.addColorStop(0,p.high);gradient.addColorStop(1,p.low);ctx.fillStyle=gradient;ctx.fillRect(0,0,W,H);circle(1030,171,96,`rgba(255,110,155,${.13*(1-p.rain*.7)})`);ctx.globalAlpha=1-p.rain*.55;circle(1030,171,65,p.sun);ctx.globalAlpha=1;ctx.fillStyle=p.low;ctx.globalAlpha=.72*(1-p.rain);for(const [x,y,w] of [[974,191,112],[984,206,92],[1000,222,60]])ctx.fillRect(x,y,w,5);ctx.globalAlpha=1;
 ...
 if(p.rain)for(let i=0;i<5;i++){const x=((i*339-s.world*.13)%(W+400)+(W+400))%(W+400)-200,y=104+(i%3)*44;ctx.globalAlpha=.27*p.rain;for(const [dx,dy,r] of [[0,13,55],[55,-8,72],[113,5,66],[169,15,49]])circle(x+dx,y+dy,r,'#10162f');ctx.globalAlpha=1}
...
function surfer(y,angle){
 ...
 const bend=clamp(s.tuck+s.landing*.18,0,1),sway=Math.sin(s.clock*6)*(1-bend);
 poly([[-59,11],[-44,5],[34,5],[63,8],[39,16],[-45,16]],C.cream);
 line([[-44,13],[50,12]],C.pink,4);line([[-44,7],[48,7]],'#fff8b4',1.5);
 const hip=[4,-39+13*bend],rk=[mix(-12,-32,bend),-19],fk=[mix(14,33,bend),-18],shoulder=[-5,-73+22*bend];
 bone([-28,1],rk,10);bone(rk,hip,12);bone([25,1],fk,10);bone(fk,hip,12);
 bone([-28,1],[-37,3],6,SKIN);bone([25,1],[36,3],6,SKIN);
 bone(hip,shoulder,19);poly([[-14,-66+21*bend],[-5,-68+21*bend],[4,-46+13*bend],[-1,-38+13*bend]],'#1c697b');
 line([[-6,-60+19*bend],[3,-43+13*bend]],C.teal,2.4);
 const elbow=[-31,-51+17*bend+sway],hand=[-49,-62+31*bend];
 bone([shoulder[0]-7,shoulder[1]+5],elbow,9);bone(elbow,[hand[0]+8,hand[1]+3],7);bone([hand[0]+8,hand[1]+3],hand,5,SKIN);
 const rs=[shoulder[0]+9,shoulder[1]+6],re=[mix(27,35,s.grab),mix(-52+15*bend-sway,-24,s.grab)],rh=[mix(44,49,s.grab),mix(-34+18*bend,8,s.grab)];
 bone(rs,re,9);bone(re,[rh[0]-6,rh[1]-3],7);bone([rh[0]-6,rh[1]-3],rh,5,SKIN);
 if(s.grab>.95)line([[46,8],[54,10]],SKIN,3);
 const head=[shoulder[0]-4,shoulder[1]-19];circle(head[0],head[1],14,SKIN);
 poly([[head[0]-14,head[1]-4],[head[0]-13,head[1]-12],[head[0]-3,head[1]-17],[head[0]+9,head[1]-13],[head[0]+12,head[1]-6],[head[0]+2,head[1]-8],[head[0]-8,head[1]-4]],C.ink);
 circle(head[0]+8,head[1],1.6,C.ink);line([[head[0]+9,head[1]+8],[head[0]+15,head[1]+6]],C.mint,1.5);
 ctx.restore()
}
function logo(){ctx.save();ctx.font='italic 900 31px Arial Black, Arial, sans-serif';ctx.textBaseline='top';ctx.lineJoin='round';const title='nachommmartinez';ctx.lineWidth=4;ctx.strokeStyle='rgba(255,50,138,.65)';ctx.strokeText(title,53,32);ctx.fillStyle=C.teal;ctx.fillText(title,49,29);ctx.fillStyle=C.cream;ctx.globalAlpha=.75;ctx.fillText(title,50,28);ctx.restore()}
