...
function updateAudio(p,dt){
 const phase=p.phase,storm=p.rain,night=p.night,nightMix=smooth((phase-.8)/.8)*(1-smooth((phase-2.6)/.5)),weights=[Math.max(0,1-nightMix-storm),nightMix,storm],flourish=clamp(s.giantSuccess/2.5,0,1),duck=1-.86*p.giant*(1-flourish);
 ...
 g.oceanGain.gain.setTargetAtTime(p.giant*(1-flourish)*.3,t,.35);
...
function render(){const p=weather();background(p);wave(p);let y=s.displayY;if(!s.airborne&&!s.crashed){const glow=clamp(.2+s.combo*.18+s.giantSuccess*.2,.2,.95),trail=[];for(let j=0;j<19;j++){const x=X-20-j*18;trail.push([x,top(x)+31+3*Math.sin(s.world*.024+j)])}ctx.globalAlpha=glow*.35;line(trail,'#ff2e93',9);ctx.globalAlpha=glow;line(trail,'#ff80bd',3.5);for(let i=0;i<15;i++){const x=X-30-i*21,yy=top(x)+24-(i*13)%14;ctx.globalAlpha=glow*(.25+(i%3)*.16);circle(x,yy,1.5+(i%3),'#ffeaf5')}ctx.globalAlpha=.12;circle(X,y+12,20+s.landing*22,p.foam);ctx.globalAlpha=1}surfer(s.crashed?Math.min(y+s.crashTime*160,H+50):y,s.crashed?1.1+s.crashTime*3:s.displayAngle);if(p.rain){ctx.fillStyle=`rgba(31,54,79,${.14*p.rain})`;ctx.fillRect(0,0,W,H);for(let i=0;i<95;i++){const x=((i*157-s.clock*155)%(W+70)+(W+70))%(W+70)-35,y=((i*89+s.clock*(490+(i%4)*80))%(H+90)+(H+90))%(H+90)-45;line([[x,y],[x-8,y+20]],`rgba(195,225,245,${.36*p.rain})`,1.5)}}hud()}
