...
function setupAudio(){
 ...
 try{
  ...
  audioGraph={ac,master,filter,trackGains,pads,rainGain,leadOsc,leadGain,bassOsc,bassGain,melodyStep:-1};ac.resume().catch(()=>{})
 }catch(e){audioGraph=null}
}
function updateAudio(p,dt){
 const phase=p.phase,storm=p.rain,night=p.night,nightMix=smooth((phase-.8)/.8)*(1-smooth((phase-2.6)/.5)),weights=[Math.max(0,1-nightMix-storm),nightMix,storm];
 if(!audioGraph){tracks.forEach((track,i)=>track.volume=music.muted?0:.32*weights[i]);return}
 ...
 trackGains.forEach((gain,i)=>gain.gain.setTargetAtTime(.75*weights[i],t,.65));
...
function reset(){s={world:0,speed:370,airY:0,airV:0,angle:0,spin:0,airborne:false,holding:false,crashed:false,crashTime:0,distance:0,nextMilestone:100,score:0,combo:0,clock:0,pose:0,tuck:0,grab:0,landing:0,displayY:0,displayAngle:0,message:'100 M = 100 PUNTOS · BACKFLIP = 250+',messageTime:5};s.displayY=surfaceY(X);gameOver?.classList.add('hidden')}
