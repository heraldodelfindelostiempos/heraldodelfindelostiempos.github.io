...
function reset(){s={world:0,speed:340,face:.49,lift:0,faceVelocity:0,airY:0,airV:0,angle:0,spin:0,airborne:false,holding:false,launchReady:false,crashed:false,crashTime:0,score:0,combo:0,best:s?.best||0,clock:0,pose:0,tuck:0,grab:0,landing:0,displayY:0,displayAngle:0,message:'MANTÉN PARA BAJAR · SUELTA PARA SUBIR',messageTime:5};s.displayY=ridingY(X)}
...
function ridingY(x){return top(x)+depth(x)*(1-s.face)-12*s.face}
function hold(value){if(s.crashed){if(value&&s.crashTime>.55)reset();return}if(value&&!s.airborne)s.launchReady=false;else if(!value&&s.holding&&!s.airborne)s.launchReady=true;s.holding=value}
...
function step(dt){s.clock+=dt;s.pose=mix(s.pose,s.holding?1:0,clamp(dt*5,0,1));s.tuck=mix(s.tuck,s.crashed?0:s.airborne?1:s.pose*.85,clamp(dt*7,0,1));s.grab=mix(s.grab,s.airborne&&!s.crashed?1:0,clamp(dt*9,0,1));s.landing=Math.max(0,s.landing-dt*2.5);if(s.crashed){s.crashTime+=dt;return}
 s.world+=s.speed*dt;s.messageTime=Math.max(0,s.messageTime-dt);
 if(s.airborne){s.airY+=s.airV*dt;s.airV+=760*dt;if(s.holding){s.angle+=5.2*dt;s.spin=Math.max(s.spin,s.angle)}else{const target=Math.round(s.angle/TAU)*TAU;s.angle=mix(s.angle,target,clamp(dt*5,0,1))}const target=ridingY(X);if(s.airV>0&&s.airY>=target){const alignment=Math.abs(((s.angle+Math.PI)%TAU+TAU)%TAU-Math.PI);if(alignment<1.05){s.airborne=false;s.airY=target;s.airV=0;s.angle=0;s.landing=1;s.faceVelocity=-.14;const turns=Math.round(s.spin/TAU);if(turns>0){s.combo++;s.score+=250*turns*s.combo;s.message=`¡${turns} GIRO! ×${s.combo}`;s.messageTime=2;s.speed=Math.min(650,s.speed+45)}else s.score+=35}else wipeout()}if(s.airY>H+70&&!s.crashed)wipeout()}
 else{const desired=s.holding?-.56:.28+s.lift*.72;s.faceVelocity+=clamp(desired-s.faceVelocity,-dt*2.8,dt*2.8);s.face=clamp(s.face+s.faceVelocity*dt,.10,1.04);if(s.holding){s.lift=Math.min(1,s.lift+.8*dt);s.speed+=clamp(530+120*s.lift-s.speed,-165*dt,165*dt)}else{s.lift=Math.max(0,s.lift-.27*dt);s.speed+=clamp(335+105*s.lift-s.speed,-48*dt,48*dt)}if(s.face>=1){if(s.launchReady&&s.speed>345){s.airborne=true;s.launchReady=false;s.face=.71;s.faceVelocity=0;s.airY=top(X)-17;s.airV=-370-(s.speed-345)*.75;s.angle=0;s.spin=0;s.lift=0}else{s.face=.88;s.faceVelocity=0}}}
...
function background(p){const gradient=ctx.createLinearGradient(0,0,0,450);gradient.addColorStop(0,p.high);gradient.addColorStop(1,p.low);ctx.fillStyle=gradient;ctx.fillRect(0,0,W,H);circle(1030,171,96,`rgba(255,110,155,${.13*(1-p.rain*.7)})`);ctx.globalAlpha=1-p.rain*.55;circle(1030,171,65,p.sun);ctx.globalAlpha=1;ctx.fillStyle=p.low;ctx.globalAlpha=.72*(1-p.rain);for(const [x,y,w] of [[974,191,112],[984,206,92],[1000,222,60]])ctx.fillRect(x,y,w,5);ctx.globalAlpha=1;
 ...
 for(let i=0;i<8;i++){const x=((i*177-s.world*.10)%(W+180)+(W+180))%(W+180)-90,h=35+(i*23)%47;ctx.fillStyle='#241d47';ctx.fillRect(x,400-h,52+(i%3)*15,h+28);if(i%2===0){ctx.fillStyle='#fd83aa';ctx.fillRect(x+11,409-h,4,5)}}for(let i=0;i<3;i++){const x=((i*670+105-s.world*.16)%(W+230)+(W+230))%(W+230)-115;palm(x,428,1+(i%2)*.26)}
